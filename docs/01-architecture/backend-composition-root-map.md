...
`api/_backend.js` exports 76 names. Twelve public handlers and scheduled
endpoints import it. The first extraction reduced it from 1,619 to 1,544 lines
by moving generic HTTP helpers to `api/_http.js`. It remains the wiring point
for routing, geocoding, fuel providers, scoring and alerts, but still owns
provider loading, prediction-domain logic and retention orchestration.
...
| HTTP and request utilities | `api/_http.js` | response object and `fetch` | Extracted; re-exported unchanged from `_backend.js` |
...
- Keep all 76 current `_backend.js` export names available during extraction.
...
- Keep the 58-file backend inventory fully required; quarantine remains zero.
...
The leaf request helpers are extracted into `api/_http.js` while their existing
