...
| P0 | `api/_addressIndex.js` | Storage configuration, connections and API transport are extracted; SQL statements and result shaping remain coupled to query orchestration | Move SQLite and Postgres query execution behind adapter methods without changing ranking or fixtures |
