...
| Backend saved routes | Backend alert storage | Bounded, installation-scoped reads | Keep while backend route alerts are enabled. Turning off the final watched route or using Delete my alert data removes the installation's routes, push devices and evaluation history atomically. | Physical-device deletion and retry behaviour still needs validation before public enablement. |
