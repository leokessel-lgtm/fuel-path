...
Saved route notifications are route-level watches, not generic reminders. A watched route may store a selected `vehicleId`, commute days, local alert time and minimum saving. Existing saved routes without these fields must keep working by falling back to the active vehicle and all-day alert checks. New saved routes should default to the active vehicle and weekday commute days. User-facing notification copy should say Fuel Path alerts only when a saving is worth checking, with fresh prices and sensible detours. Do not expose backend, token or sync terminology in Settings copy.
