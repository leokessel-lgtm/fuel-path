...
- The 12 July hosted alert-sync report targeted Production's merged PR #29 API, not PR #30 Preview. It therefore does not prove the account-free backend contract. Its route was deleted, and its synthetic push-device row was explicitly marked `inactive`; PR #29 exposes no public delete-device endpoint, so the inert row remains. The smoke harness now refuses Production by default, uses installation id and secret credentials, authenticates reads, atomically deletes its temporary installation data and verifies capability revocation. It still needs a run against an isolated PR #30 Preview deployment with capability issuing configured.
