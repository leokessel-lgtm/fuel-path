...
- Real Expo push-token creation is not proved. The local APK does not contain `google-services.json`; that file is EAS-secret build input. A current-head EAS Preview Android build was attempted on 13 July but rejected because the account's monthly Android build quota was exhausted. Older EAS APKs predate PR #30 and are not substitutes for current account-free evidence.
