...
- Root beta readiness with provider, store and support evidence now passes for controlled real-user testing after this physical Android performance run: `node scripts/check-beta-readiness.mjs --provider-terms-evidence-json PROVIDER-TERMS-EVIDENCE-2026-07-05.json --store-evidence-json STORE-PUBLISHING-EVIDENCE-2026-07-05.json --support-evidence-json SUPPORT-READINESS-EVIDENCE-2026-07-05.json --allow-blocked` returned `status: ready`, zero blockers and `performanceClaimAllowed: true`. Public live-region claims remain WA-only, and broader native launch claims still need the separate iOS/device evidence called out below.
