...
Last reviewed: 13 July 2026, Australia/Sydney.
...
- Physical map/performance smoke: `tmp/native-smoke/android-preview-smoke-2026-07-12T22-14-39-030Z.md`, status `passed`. The installed current-checkout APK rendered Plan, Nearby, pan and Settings with live Google map tiles and no map credential warnings. The measured pass recorded 327 frames, 5 janky frames, 1.5% jank and p95 10 ms. Summary: `tmp/native-smoke/android-performance-summary-2026-07-12T22-41-33-874Z.md`, status `passed`.
- Performance claim remains `controlled_beta_only`: `tmp/native-smoke/android-performance-coverage-2026-07-12T22-41-33-974Z.md`. Pixel 9 Pro is high-end evidence; a current lower or mid-range physical Android pass is still required for broad Android performance claims.
- Cold start and foreground resume: `tmp/native-smoke/android-cold-start-smoke-2026-07-12T22-16-26-276Z.md`, status `passed`. Five of five settled launch captures and the foreground-resume capture passed, with zero runtime failure lines and zero map warnings. This is not deep process-death restoration evidence.
- Broad physical UI traversal: `tmp/native-checklist/native-visual-checklist-2026-07-12T22-18-59-437Z.md` covered Plan, Nearby, map pan, sort, marker detail and all Settings sections. Its original Plan destination failure was a harness defect: substring matching treated `To` as `Go to Nearby`. The harness now uses exact string selection and a focused `--plan-only` mode. The repaired rerun `tmp/native-checklist/native-visual-checklist-2026-07-12T22-24-50-286Z.md` passed destination resolution, route result, recommendation detail and marker detail.
- Current EV Plan parity packet: `tmp/native-smoke/android-pixel-ev-route-result-2026-07-12T22-54-50-155Z.png` and `tmp/native-smoke/android-pixel-ev-route-result-2026-07-12T22-54-50-155Z.xml`. The focused device journey passed 7 of 7 checks, including finding both autocomplete suggestions rather than using fallback coordinates. EV was selectable from `PLAN WITH` as `EV charge`; the vehicle chip displayed `EV | 400 km | Connectors not set`; Sylvania NSW and Newcastle NSW suggestions appeared; `Check route range` returned `Route charger options`, `10 options`, `178 km route. 400 km selected range`, visible charger rows and route-map charger markers. Android CLI layout inspection independently found the same route, count and range text; its screenshot is `tmp/native-smoke/android-cli-ev-route-result-2026-07-13.png`.
- Notification packaging and device state: `tmp/native-smoke/android-notification-readiness-2026-07-12T22-18-08-485Z.md`, status `passed`. The Pixel showed the Android permission prompt, permission was granted, `dumpsys package` reported `POST_NOTIFICATIONS: granted=true`, and Fuel Path reconciled to `Permission enabled`. Screenshot: `tmp/native-smoke/android-notification-permission-enabled-2026-07-13.png`.
- Real Expo push-token creation is now proved on the Pixel 9 Pro without printing or persisting the token in evidence. Expo Starter capacity produced current-head internal Preview APK build `e7dba6d3-3334-4577-a67e-9d962708cba8` from commit `fa63372`; the downloaded APK is `mobile-app/native-artifacts/fuel-path-preview-android-fa63372-e7dba6d3.apk`, SHA-256 `7acc91bf687fbb1aa92788ff9c098e6afebcfb7e7177a8a7a0838d8fb3285546`. The APK contains the active Firebase Android app id, sender id and project id for `com.fuelpath.app`; Firebase initialised successfully; EAS has an FCM v1 service account assigned; Android notification permission was granted through the real UI; and the saved-route Watch flow advanced past Expo token creation into backend sync. The initial run exposed concurrent token registration between the user action and native token-change listener, producing repeated cancelled Expo updater requests. Commit `fa63372` serialises token creation and rate-limits rotation refresh; the correction APK reduced the updater warning count to zero. Backend sync then failed with the expected Production-side message because this Preview-profile APK still targets Production, where PR #30 account-free capability issuing is not deployed. This proves token creation, not delivered push notification receipt.
- The same physical flow exposed an unreachable Save/Why action row in the native Plan result sheet. Commit `a90a4fb` moved those actions ahead of the tall recommendation card and added a source guard. Correction build `d53874e7-74b5-442b-9fd9-ac3c556a8378` proved `Save route` and `Show route evidence` were present and actionable at the Pixel's normal density and font scale. No scoring, savings or route recommendation logic changed.
- Hosted push delivery remains `blocked_by_environment`: `tmp/native-smoke/android-alert-delivery-gate-2026-07-12T22-18-12-526Z.md`. The production backend is reachable and durable, but the Expo delivery gate is disabled. No real notification was sent or observed in the tray.
- Account-free backend lifecycle now passes against the isolated PR #30 Preview deployment. On 13 July, Preview-only branch-scoped `ALERTS_CLIENT_WRITE_ENABLED` and `ALERTS_CLIENT_CAPABILITY_SECRET` values were configured for `codex/account-free-native-foundation`; Production was not changed. The protected Preview reported durable Neon storage, capability issuing enabled and zero starting device/route rows. An authenticated lifecycle issued a scoped capability, registered a token-shaped Android test device, saved/listed/deleted a P95 route watch, atomically deleted the installation's alert data, revoked the capability, and finished with zero device and route rows. No push was sent. The run also exposed and repaired a smoke-harness defect: generated installation ids used the invalid `readiness_installation_` prefix even though the backend requires `installation_`; the harness now generates `installation_readiness_` and the source guard locks that contract. One capability-only diagnostic installation was created before that defect was isolated; it has no device or route data, but the public API cannot delete it without its now-discarded installation secret.
- The 12 July hosted alert-sync report targeted Production's merged PR #29 API, not PR #30 Preview. It remains excluded from PR #30 evidence. Its route was deleted, and its synthetic push-device row was explicitly marked `inactive`; PR #29 exposes no public delete-device endpoint, so the inert row remains. The smoke harness refuses Production by default, uses installation id and secret credentials, authenticates reads, atomically deletes its temporary installation data and verifies capability revocation.
