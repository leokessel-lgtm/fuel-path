...
export function NearbyScreenLogic({
  preferences,
  onFuelChange,
  onAddRecentLocation,
  persistedCentre,
  ...
  persistedCentre?: MapPoint;
  onPersistCentre?: (centre: MapPoint) => void;
  onCurrentLocationChange?: (centre: MapPoint) => void;
}) {
  const [centre, setCentre] = useState<MapPoint>(() => persistedCentre || defaultNearbyCentre);
  const [currentLocation, setCurrentLocation] = useState<MapPoint | undefined>(() =>
    persistedCentre?.label === "Current location" ? persistedCentre : undefined,
  );
  ...
  const {
    ...
    locationSuggestions,
    recentLocations: nearbyHookRecentLocations,
  ...
  const locationSelectionRef = useRef<"field" | null>(null);
  const withLocationSelectionSilence = (action: () => void) => {
    locationSelectionRef.current = "field";
    action();
    setTimeout(() => {
      if (locationSelectionRef.current === "field") {
        locationSelectionRef.current = null;
      }
    }, 0);
  };
  ...
  const quickLocations = useMemo(() => {
    type NearbyQuickLocation = MapPoint & { kind: "home" | "work" | "recent" };
    const quickList: NearbyQuickLocation[] = [];
    const seen = new Set<string>();
    const pushQuick = (
      point: MapPoint | undefined,
      kind: NearbyQuickLocation["kind"],
      label: string,
      subtitle: string,
    ) => {
      if (!point) return;
    ...
    pushQuick(preferences.homeLocation, "home", "Home", preferences.homeLocation?.label || "Home");
    pushQuick(preferences.workLocation, "work", "Work", preferences.workLocation?.label || "Work");
    const fallbackRecents = recentLocations?.length ? recentLocations : nearbyHookRecentLocations;
    for (const location of fallbackRecents ?? []) {
      pushQuick(location, "recent", location.label, location.label);
    }
    return quickList;
  }, [preferences.homeLocation, preferences.workLocation, nearbyHookRecentLocations, recentLocations]);
  ...
  const applyLocationSearch = async () => {
    ...
    try {
      ...
      onAddRecentLocation?.(nextCentre);
      clearLocationSearch();
      resetAddressSessionToken();
      setNearbySheetSnap("browse");
    } catch (err) {
  ...
  const useCurrentLocation = async () => {
    ...
    try {
      ...
      setCentre(nextCentre);
      onPersistCentre?.(nextCentre);
      onCurrentLocationChange?.(nextCentre);
      setNearbyRadiusKm(defaultNearbyRadiusKm);
      setCameraFocusVersion((current) => current + 1);
      setLocationQuery("");
      clearLocationSearch();
  ...
  const selectRecentLocation = (location: MapPoint) => {
    withLocationSelectionSilence(() => {
      setCentre(location);
      onPersistCentre?.(location);
      setNearbyRadiusKm(defaultNearbyRadiusKm);
      setCameraFocusVersion((current) => current + 1);
      setLocationQuery(location.label);
      onAddRecentLocation?.(location);
      clearLocationSearch();
      setNearbySheetSnap("browse");
    });
  };
  ...
  const handleLocationBlur = () => {
    setTimeout(() => {
      if (locationSelectionRef.current === "field") return;
      clearLocationSearch();
    }, 120);
  };
    ...
    <View style={styles.screen}>
      ...
      <View onLayout={onTopControlsLayout} style={styles.topControls}>
        <NearbyLocationSearch
          ...
          onBeginSelection={beginLocationSelection}
          ...
          onActivateSearch={activateLocationSearch}
          onSelectSuggestion={selectLocationSuggestion}
          onSelectQuickLocation={selectRecentLocation}
