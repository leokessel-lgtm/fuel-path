...
import { usePlanCurrentLocationSync } from "./usePlanCurrentLocationSync";
import {
...
export function PlanScreenLogic({
  ...
  const fromPointIsCurrentLocation = Boolean(
    currentLocation &&
      ...
      Number.isFinite(fromPoint.lon) &&
      Math.abs(currentLocation.lat - fromPoint.lat) < 0.00015 &&
      Math.abs(currentLocation.lon - fromPoint.lon) < 0.00015,
  );
  ...
  const handleFromChange = (value: string) => {
    ...
    setFrom(value);
    const fromLabelMatch = fromPoint
      ? value.trim().toLowerCase().replace(/[^a-z0-9]+/g, " ").trim() === fromPoint.label.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim()
      : false;
    if (!fromSelectionSuppressRef.current && !fromLabelMatch) {
  ...
  const handleToChange = (value: string) => {
    ...
    setTo(value);
    const toLabelMatch = toPoint
      ? value.trim().toLowerCase().replace(/[^a-z0-9]+/g, " ").trim() === toPoint.label.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim()
      : false;
    if (!toSelectionSuppressRef.current && !toLabelMatch) {
  ...
  const selectAddressSuggestion = async (field: "from" | "to", point: MapPoint) => {
    ...
    if (field === "from") {
      ...
      setTimeout(() => setFromPoint(resolvedPoint), 0);
    } else {
      setTo(label);
      setToPoint(resolvedPoint);
      setTimeout(() => setToPoint(resolvedPoint), 0);
    }
  ...
  const useCurrentFromLocation = async () => {
    ...
    try {
      ...
      onCurrentLocationChange?.(nextFromPoint);
      reopenRouteEditor();
      setActiveAddressField(null);
      resetAddressSessionToken("from");
    } catch (err) {
    ...
    <View style={styles.screen}>
      <View style={styles.mapLayer}>
        <StationMap
          ...
          showCentreMarker={Boolean(fromPoint) && !fromPointIsCurrentLocation}
          userLocation={fromPointIsCurrentLocation ? fromPoint : undefined}
        />
      </View>
