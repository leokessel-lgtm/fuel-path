...
const jsonPath = path.join(evidenceDir, `prediction-backtest-proof-${runId}.json`);
...
const hardBlockers = predictionProofHardBlockers(summary);
if (hardBlockers.length && !args.allowBlocked) {
  throw new Error(`Prediction proof gate blocked: ${hardBlockers.join(", ")}`);
}
...
function predictionProofHardBlockers(summary) {
  return [
    ...(summary.userFacingPredictionEnabled ? ["user_facing_prediction_enabled"] : []),
    ...(summary.accuracyClaimsAllowed ? ["accuracy_claims_allowed"] : []),
    ...(summary.blindSpotExposure === "api_blind_spots_missing" ? ["api_blind_spots_missing"] : []),
    ...(summary.failedSignalChecks ? ["signal_checks_failed"] : []),
  ];
}
